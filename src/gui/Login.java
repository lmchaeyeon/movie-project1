...
public class Login {
    ...
    private void initialize() {
        ...
        lblNewLabel.setFont(new Font("Arial", Font.BOLD, 13)); // 기본 폰트 사용
        lblNewLabel.setBounds(60, 97, 59, 22);
        startPage.add(lblNewLabel);

        JLabel lblPw = new JLabel("PW");
        lblPw.setFont(new Font("Arial", Font.BOLD, 13)); // 기본 폰트 사용
        ...
        lblNewLabel_1.setBounds(174, 10, 103, 77);
        startPage.add(lblNewLabel_1);
        
        JLabel lblNewLabel_2 = new JLabel("Lcymoive");
        lblNewLabel_2.setFont(new Font("한컴산뜻돋움", Font.BOLD, 17));
        lblNewLabel_2.setBounds(174, 288, 77, 30);
        ...
        btnJoinBack.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (isInputEmpty) {
                    showMessageDialog("Error", "빈 칸이 있습니다.");
                } else if (!String.valueOf(passwordField.getPassword())
                        .equals(String.valueOf(passwordField_1.getPassword()))) {
                    showMessageDialog("Error", "PW가 일치하지 않습니다.");
                } else {
                    register(joinIdField.getText(), String.valueOf(passwordField.getPassword()), textField_1.getText(),
                            textField_3.getText());
                }
            }
        });
        ...
        btnNewButton_1.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (id.isEmpty()) {
                    showMessageDialog("Error", "아이디를 입력하세요.");
                } else if (checkDuplicateId(id)) {
                    showMessageDialog("Error", "이미 사용 중인 ID입니다.");
                } else {
                    showMessageDialog("Success", "사용 가능한 ID입니다.");
                }
            }
        });
    ...
    private void login() {
        ...
        try (Connection conn = DB1.getConnection()) {
            ...
            if (rs.next()) {
                showMessageDialog("Success", "로그인 성공!");
                ...
                showMessageDialog("Error", "아이디 또는 비밀번호가 잘못되었습니다.");
            }
    ...
    private void register(String id, String password, String name, String phone) {
        try (Connection conn = DB1.getConnection()) {
            String query = "INSERT INTO users (username, password, name, phone) VALUES (?, ?, ?, ?)";
            PreparedStatement pstmt = conn.prepareStatement(query);
            pstmt.setString(1, id);
            pstmt.setString(2, password);
            ...
            pstmt.executeUpdate();
            showMessageDialog("Success", "회원가입 성공!");
            ...
            e.printStackTrace();
            showMessageDialog("Error", "회원가입 실패: " + e.getMessage());
        }
    }
